...
summary: "A guide to setting up tRPC with Svelte Query in a Svelte 5 project."
tags:
  - svelte
  - tutorial
---
...
tRPC is a tool for building type-safe APIs with TypeScript. I like it, because it provides a really pleasant framework for you to build an API in a SvelteKit application. 
...
For its SvelteKit equivalent, <Link href="https://github.com/icflorescu/trpc-sveltekit">tRPC-SvelteKit</Link>, the implementation is not that easy. The integration with <Link href="https://tanstack.com/query/latest/docs/framework/svelte/overview">Svelte Query</Link> is more complex. Moreover, it is not updated for **Svelte 5** syntax.
...
If you are using Svelte 5, you should already have the `src/lib` directory. We will use it to store the tRPC files.

First start with creating the `src/lib/trpc/t.ts` file:
...
This will create a tRPC instance that will be used to create the API endpoints.

Next, create the `src/lib/trpc/context.ts` file:
...
Next, I recommend creating a `src/lib/trpc/router` directory, and inside it create the `index.ts` file:
...
In this file, you can add all the sub-routers you need. For example, we can create a `posts` sub-router:
...
For server-side data loading, you can use the `load` function in your SvelteKit page. You have two options here:

1. Create a separate server side api function, which will call the router function directly.
